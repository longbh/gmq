...
func (ua *UnsubackPacket) Write(w io.Writer) error {
	...
	packet.Write(utils.EncodeUint16(ua.MessageID))
...
func (ua *UnsubackPacket) Unpack(b io.Reader) error {
	var err error
	ua.MessageID, err = utils.DecodeUint16(b)
...
//header
func (pr *UnsubackPacket) FixHeader() FixedHeader {
	return pr.FixedHeader
}

func (ca *UnsubackPacket) Process() ControlPacket {
	control := NewControlPacket(Unsubscribe)
	return control
}